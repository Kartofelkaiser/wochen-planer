...
## Rezept-Herkunft
- **Rezepte nicht selbst ausdenken, sondern KOMPLETT (1:1) aus dem Internet übernehmen** – möglichst gut und oft bewertete Rezepte (z. B. Chefkoch, EAT SMARTER, kochbar, gaumenfreundin, TheMealDB), nur Portionsgröße auf 1 Person skalieren und ggf. übersetzen. **Gilt für ALLE Mahlzeiten: Frühstück, Mittag- und Abendessen.** Wenn die Arbeitsumgebung die Rezeptseite nicht abrufen kann, das offen sagen und die originalgetreueste mögliche Fassung liefern (Kennzeichnung „nach Rezept: Quelle"), 1:1-Abgleich nachholen, sobald Seitenzugriff besteht.
...
## Aktueller Stand (06.07.2026)
- **Neuer Wochenplan Mo 06.07.–So 12.07.** (komplett neue Internet-Rezepte, 3 Optionen/Tag, Meal-Prep wo passend): Mo **Hähnchen-Gyros mit Tzatziki & Reis** · Di **Penne Arrabiata mit Kichererbsen** · Mi **Ofen-Hackbällchen in Tomatensauce mit Nudeln** · Do **Kedgeree (Curryreis mit Räucherfisch & Ei)** · Fr **Pizza Margherita** · Sa **Corba – türkische rote Linsensuppe mit Brot** · So **Hähnchen-Parmigiana mit Reis**. Frühstück: **Milchreis bleibt** (Default), neue Optionen Bircher Müsli, Skyr-Bowl, Avocado-Ei-Brot, Bratapfel-Joghurt. Mittag (5 neu): Mediterraner Nudelsalat (Default), Bulgur-Bowl, Kartoffelsalat mit Ei, Linsensalat, Möhren-Ingwer-Suppe. Quellen: TheMealDB (Arrabiata, Kedgeree, Corba, Ratatouille, Med. Nudelsalat) + EAT SMARTER/gaumenfreundin (Gyros, Hackbällchen, Parmigiana, Frühstücke, restl. Mittage).
- Umsetzung: neue Woche via einmaliger Migration `plan.appliedWeek='2026-07-06'` (setzt `plan.dinners/breakfast/lunch`, löscht alte `byDate`/`applied10day`, setzt `finalized_v1` auf gestern für sauberen Start). Gerichte sind fest in `DISHES` (Präfix `gen-`) gebacken – kein `custom_v4` nötig. PACKS um Bulgur/Mozzarella/Skyr/Räucherfisch/Semmelbrösel/Oliven/Rosinen ergänzt.
- ⚠️ **Live-Site war beim Erstellen down (HTTP 404 auf ALLES, auch `/` – bleibt auch nach frischem Push 404).** Ursache liegt NICHT am Code, sondern bei Netlify (Repo-Anbindung getrennt / Site umbenannt oder gelöscht). Maik muss Netlify prüfen (Site ↔ GitHub-Repo `Kartofelkaiser/wochen-planer` neu verbinden bzw. Deploy anstoßen). Solange 404: Store-API (`/api/store`) nicht lesbar/schreibbar → Live-Vorrat konnte nicht gelesen werden; der neue Plan greift automatisch, sobald die Site wieder deployt.
- Vorräte (Stand 03.07.-Seed, evtl. schon teils verbraucht, konnte nicht live gegengeprüft werden): Kartoffeln 1800 g, Hähnchenbrust 700 g, Milch 700 ml, Käse gerieben 380 g, Magerquark 500 g, Apfelmus 710 g, Milchreis 500 g. Die App zieht den echten „Vorräte"-Bestand automatisch von der Einkaufsliste ab, sobald sie wieder online ist.
- **Einkaufsliste und „Plan bearbeiten" haben einen Wochen-Umschalter** („Diese Woche (ab heute)" / „Nächste Woche"); die Einkaufsliste startet auf „Diese Woche".
